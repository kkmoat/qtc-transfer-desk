...
可公开下载的 [npm 运行依赖许可证与版权全文](public/source/npm-LICENSES.txt) 汇总了当前锁文件中未标记为仅开发依赖的 113 个包，包括生产依赖树中的间接、peer 和类型包；其中 112 个包附有原始许可证文件全文。`react-remove-scroll-bar@2.3.8` 的安装包未提供许可证文件，汇总中仅记录该版本声明的 MIT 许可证与上游地址，未补造文本。该汇总保留原版权声明，并标注来源文件和校验和；更新依赖后需要同步重新生成。
