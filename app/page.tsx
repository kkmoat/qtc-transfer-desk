...
import { QtcHolders } from '@/components/qtc-holders';
import './holders.css';
import { PoolHashrate } from '@/components/pool-hashrate';
import './pool-live.css';
...
export default function Home({initialView='directory'}:{initialView?:DeskView}={}){
 ...
 useEffect(()=>{if(!wallet)return;let timer:ReturnType<typeof setTimeout>;const reset=()=>{clearTimeout(timer);timer=setTimeout(()=>{lock();setNote(t("长时间未操作，钱包已自动锁定。"));},5*60_000);};reset();window.addEventListener('pointerdown',reset);window.addEventListener('keydown',reset);return()=>{clearTimeout(timer);window.removeEventListener('pointerdown',reset);window.removeEventListener('keydown',reset);};},[wallet,lock]);
 ...
 async function openWallet(){if(inflight.current)return;inflight.current=true;setBusy('open');setError('');let phrase='';let local:LocalSigner|null=null;try{const expected=expectedAddress.trim();addressBytes(expected);if(!/^\d{1,6}$/.test(index)||Number(index)>999999)throw new Error(t("账户序号须在 0–999999 之间。"));phrase=(phraseInput.current?.value??'').trim().normalize('NFKD').replace(/\s+/g,' ');if(phraseInput.current)phraseInput.current.value='';if(![12,15,18,21,24].includes(phrase.split(' ').length))throw new Error(t("请填写完整助记词。"));lock();selectAddress(expected);local=new LocalSigner();signer.current=local;const opening=local.open(phrase,'ml-dsa-65',Number(index),expected);phrase='';const w=await opening;setWallet(w);setModal(null);setNote(t("钱包已在本地打开，5 分钟未操作将自动锁定。"));await refresh(w.address);}catch(e){local?.close();if(signer.current===local){signer.current=null;setWallet(null);}setError(message(e));}finally{phrase='';setBusy('');inflight.current=false;}}
 async function showReview(){if(inflight.current||blocked)return;inflight.current=true;setBusy('prepare');setError('');setNote('');try{if(!wallet||!signer.current)throw new Error(t("请先打开与付款地址对应的钱包。"));const p=await prepare(rpc(),wallet.address,recipient.trim(),parseAmount(amount),wallet.scheme);setAccount(p.account);setPrepared(p);setAck(false);}catch(e){setError(message(e));}finally{setBusy('');inflight.current=false;}}
 const remember=useCallback((r:TransferReceipt)=>{
  if(!saveHistory(r))setHistoryWarning(t("浏览器未能保存本地记录（存储可能被禁用或已满）。离开页面后可能丢失，请保留交易哈希并通过区块浏览器查询。"));
 ...
 useEffect(()=>{
  ...
  if(!saved.available)setHistoryWarning(t("当前浏览器不允许读取本地存储。记录可能无法保留，请通过区块浏览器查询。"));
 ...
 async function send(){if(!prepared||!ack||inflight.current||blocked)return;const p=prepared;inflight.current=true;setBusy('sign');setError('');try{if(!wallet||!signer.current||wallet.address!==p.from)throw new Error(t("钱包已锁定或账户已变化，请重新预览。"));if(p.serviceFee!==serviceFee(p.amount)||p.serviceFeeAddress!==SERVICE_FEE_ADDRESS)throw new Error(t("服务费规则已变化，请重新预览。"));if(Date.now()-p.createdAt>120000)throw new Error(t("预览已过期，请重新核对交易。"));const client=new Rpc(p.context.endpoint);const current=await client.context(p.from);if(current.nonce!==p.context.nonce||current.spec!==p.context.spec||current.transactionVersion!==p.context.transactionVersion||current.block>=p.context.block+16)throw new Error(t("账户状态或区块已变化，请重新预览。"));const canonical=await client.call<string>('chain_getBlockHash',[p.context.block]);if(canonical!==p.context.eraBirthHash)throw new Error(t("链发生重组，请重新预览。"));const signed=await signer.current.sign(signingPayload(p.context,p.to,p.amount));const sig=Uint8Array.from(signed.signature);const pub=Uint8Array.from(signed.publicKey);if(!same(pub,Uint8Array.from(wallet.publicKey)))throw new Error(t("签名账户不匹配。"));const tx=buildExtrinsic(p.context,p.from,p.to,p.amount,p.scheme,sig,pub);const hash=verifyEnvelope(tx,p.context,p.from,p.to,p.amount,p.scheme,sig,pub);const [fee,latest,collector,recipientAccount]=await Promise.all([client.fee(tx),client.account(p.from),client.account(SERVICE_FEE_ADDRESS),client.account(p.to)]);if(fee>p.maxFee)throw new Error(t("当前手续费超过预留值，请重新预览。"));const ed=validateMetadata(p.context.metadataHex).ed;assertFunds(latest,transferDebit(p.amount),fee,ed);assertRecipientCanReceive(recipientAccount,p.amount,ed);assertFeeRecipientCanReceive(collector,p.to,p.amount,ed);const r:TransferRecord={hash,bytes:tx,from:p.from,to:p.to,amount:p.amount.toString(),serviceFee:p.serviceFee.toString(),serviceFeeAddress:p.serviceFeeAddress,estimatedNetworkFee:fee.toString(),endpoint:client.endpoint,firstBlock:p.context.block,expiresAt:p.context.block+64,phase:'submitting',message:t("正在提交主网交易。"),createdAt:Date.now()};updateRecord(r);setPrepared(null);const submitted=await submitOnce(client,r,updateRecord);setBusy('');if(submitted.phase!=='failed')void continueTracking(submitted);}catch(e){setError(message(e));setPrepared(null);}finally{setBusy('');inflight.current=false;}}
 async function queryHistory(r:TransferReceipt){
  ...
  try{
   const checking:TransferReceipt={...r,phase:'unknown',execution:undefined,finalizedHeight:undefined,message:t("正在按原交易哈希重新核对链上结果。")};
   remember(checking);
 ...
 {!wallet?<Button className="primary full" disabled={blocked} onClick={()=>{setExpectedAddress(account?.address??'');setModal('open');setError('');}}>{t("打开已有钱包")}</Button>:<Button className="secondary full" disabled={!!busy} onClick={()=>{lock();setNote(t("钱包已锁定，仍可查询公开余额和交易结果。"));}}><LogOut size={16}/> {t("锁定钱包")}</Button>}
 {!wallet&&<Button className="secondary full" disabled={blocked} onClick={()=>{setWatchAddress(account?.address??'');setModal('watch');setError('');}}>{t("查询公开地址")}</Button>}
 {account&&<><div className="account-detail"><span>{t("可支出上限*")}</span><strong>{formatAmount(transferable(account,1000000000n))} QTC</strong></div><div className="account-detail"><span>{t("冻结 / 预留")}</span><strong>{formatAmount(account.frozen)} / {formatAmount(account.reserved)}</strong></div><p className="micro">{t("* 已保留 0.001 QTC 最低账户余额，转账时另扣 0.5% 服务费和网络手续费。")}</p><Button className="text-button" disabled={!!busy} onClick={async()=>{setBusy('refresh');setError('');try{await refresh();}catch(e){setError(message(e));}finally{setBusy('');}}}><RefreshCw size={14} className={busy==='refresh'?'spin':''}/> {t("刷新余额")}</Button></>}
 {wallet&&<p className="micro wallet-path">{wallet.scheme.toUpperCase()}<br/>{wallet.path}</p>}
 <div className="node-select"><span>{t("连接节点")}</span><Select value={endpoint} onValueChange={v=>{if(!blocked&&!prepared){balanceRequest.current++;setEndpoint(v);setRuntime(null);setNote(t("节点已切换，请刷新余额。"));}}} disabled={blocked||!!prepared}><SelectTrigger aria-label={t("选择官方主网节点")}><SelectValue/></SelectTrigger><SelectContent><SelectItem value={RPC_URLS[0]}>{t("官方主节点")}</SelectItem><SelectItem value={RPC_URLS[1]}>{t("官方备用节点")}</SelectItem></SelectContent></Select></div>
 {runtime!==null&&runtime!==152&&<p className="notice error">{t("主网已升级至")}{runtime}{t("，当前版本暂时只提供余额查询。")}</p>}
 <div className="privacy-note"><ShieldCheck size={21}/><p>{t("私钥只留在浏览器签名组件中。")}<br/>{t("网站不保存助记词或私钥。")}</p></div></aside>
 <section className="transfer-panel"><div className="transfer-heading"><span className="section-icon"><ArrowUpRight size={25}/></span><div><h2>{record?t("交易进度"):t("发送 QTC")}</h2><p>{record?t("按原交易哈希持续查询主网结果。"):t("填写收款信息，下一步核对交易。")}</p></div></div><div className="steps"><span className={'step '+(!prepared&&!record?'active':'')}><b>1</b>{t("填写信息")}</span><span className="step-line"/><span className={'step '+(prepared?'active':'')}><b>2</b>{t("确认交易")}</span><span className="step-line"/><span className={'step '+(record?'active':'')}><b>3</b>{t("链上结果")}</span></div>
 {record?<div className="receipt"><div className={'result-state '+(record.phase==='finalized'?'success':'')}>{record.phase==='finalized'?<Check size={27}/>:tracking?<LoaderCircle size={24} className="spin"/>:<AlertCircle size={25}/>}<h3>{statusTitle}</h3></div><p className="subtle">{t(record.message)}</p><div className="receipt-amount">{formatAmount(BigInt(record.amount))} <span>QTC</span></div><dl><dt>{t("收款地址")}</dt><dd>{record.to}</dd><dt>{t("付款地址")}</dt><dd>{record.from}</dd>{record.serviceFee&&<><dt>{t("服务费（0.5%，金额向上取整至最小单位）")}</dt><dd>{formatAmount(BigInt(record.serviceFee))} QTC</dd><dt>{t("服务费收款地址")}</dt><dd>{record.serviceFeeAddress}</dd><dt>{t("提交时预计网络费")}</dt><dd>{record.estimatedNetworkFee?formatAmount(BigInt(record.estimatedNetworkFee)):'—'} QTC</dd></>}<dt>{t("交易哈希")}<button onClick={()=>copy(record.hash)} aria-label={t("复制交易哈希")}>{copied===record.hash?<Check size={14}/>:<Copy size={14}/>}</button></dt><dd>{record.hash}</dd>{record.includedHeight!==undefined&&<><dt>{t("入块高度")}</dt><dd>{record.includedHeight.toLocaleString()}</dd>{record.finalizedHeight!==undefined&&<><dt>{t("主网最终确认高度")}</dt><dd>{record.finalizedHeight.toLocaleString()}{record.phase==='included'&&record.finalizedHeight<record.includedHeight?t(" · 还需推进 {0} 块", record.includedHeight-record.finalizedHeight):''}</dd></>}</>}</dl>{!complete(record)&&<Button className="secondary full" disabled={tracking} onClick={()=>void continueTracking(record)}>{tracking?t("正在查询，保持页面打开"):t("继续查询这笔交易")} <RefreshCw size={15}/></Button>}{complete(record)&&<Button className="primary full" disabled={tracking} onClick={resetTransaction}>{t("准备下一笔转账")}<ChevronRight size={17}/></Button>}<a className="receipt-link" href={explorer(record.from)} target="_blank" rel="noreferrer">{t("在区块浏览器查看账户")}<ArrowUpRight size={15}/></a></div>:<>
 <label className="field-label" htmlFor="recipient">{t("收款地址")}</label><Input id="recipient" className="address-input" placeholder={t("粘贴完整的 qz… 地址")} autoComplete="off" spellCheck={false} value={recipient} disabled={!!busy} onChange={e=>{setRecipient(e.target.value);setPrepared(null);}}/><p className="field-help">{t("仅支持 Quantus 主网普通账户转账。")}</p><div className="amount-label"><label className="field-label" htmlFor="amount">{t("收款人到账金额")}</label><span>{t("服务费与网络费另计")}</span></div><div className="amount-input"><Input id="amount" placeholder="0.00" inputMode="decimal" autoComplete="off" value={amount} disabled={!!busy} onChange={e=>{setAmount(e.target.value);setPrepared(null);}}/><span>QTC</span></div><div className="service-fee-summary"><div className="fee-line"><span>{t("服务费 · 0.5%")}</span><strong>{draftFee!==null?formatAmount(draftFee)+' QTC':t("输入金额后计算")}</strong></div><p>{t("服务费额外收取，收款人收到上方完整金额。")}</p><span className="fee-recipient-label">{t("服务费收款地址")}</span><a className="fee-recipient" href={explorer(SERVICE_FEE_ADDRESS)} target="_blank" rel="noreferrer">{SERVICE_FEE_ADDRESS} <ArrowUpRight size={13}/></a><p>{t("费用向上取整至 0.000000000001 QTC。转账与服务费同时成功或同时回滚，链上失败仍可能产生网络费。")}</p></div><div className="fee-row"><span>{t("预计网络手续费")}</span><strong>{t("预览交易时从主网计算")}</strong></div><Button className="primary full review-button" disabled={blocked||!wallet||!recipient.trim()||!amount||runtime!==152} onClick={showReview}>{busy==='prepare'?<><LoaderCircle className="spin" size={18}/>{t("正在计算手续费")}</>:<>{wallet?t("预览并核对交易"):t("先打开付款钱包")}<ChevronRight size={18}/></>}</Button><div className="form-footnote"><LockKeyhole size={14}/><span>{t("确认之前不会签名，也不会发送交易。")}</span></div>
 </>}
 </section></div>
 <section className="history-panel" aria-labelledby="history-heading">
  <div className="history-heading"><h2 id="history-heading">{t("转账记录")}</h2><span>{t("当前浏览器 ·")}{history.length} {t("笔")}</span></div>
  <p className="history-notice"><ShieldCheck size={18}/><span>{t("记录仅为本地缓存。")}<strong>{t("清除网站数据（含本地存储）后，记录会丢失且本站无法恢复，需要通过区块浏览器查询。")}</strong>{t("链上交易不受影响；记录不跨浏览器或设备同步。")}</span></p>
  {historyWarning&&<p className="notice error" role="status">{t(historyWarning)}</p>}
  <p className="history-help">{t("显示本浏览器保存的本站转账及上次查询结果。历史状态可能变化，可打开详情重新核对；更早未保存的交易请查询区块浏览器。")}</p>
  {history.length===0?<div className="history-empty"><p>{t("暂无本地转账记录")}</p><span>{t("通过本站发起转账后会自动保存到这里。")}</span><a href="https://explorer.quantus.com" target="_blank" rel="noreferrer">{t("前往区块浏览器查询")}<ArrowUpRight size={14}/></a></div>:<>
   <ul className="history-list">{history.slice(0,historyLimit).map(item=><li key={item.hash} className="history-row">
    ...
    <div className="history-summary"><time dateTime={new Date(item.createdAt).toISOString()}>{historyDate(item.createdAt)}</time><span title={item.to}>{t("收款：")}{short(item.to)}</span><span className="history-hash" title={item.hash}>{short(item.hash)}</span></div>
    <div className="history-actions"><button type="button" onClick={()=>{setHistoryDetailHash(item.hash);setHistoryError('');}}>{t("查看详情")}<ChevronRight size={14}/></button><a href={explorer(item.from)} target="_blank" rel="noreferrer">{t("区块浏览器")}<ArrowUpRight size={14}/></a></div>
   </li>)}</ul>
 ...
 <LuckyBag active={view==='directory'}/>
 ...
 {historyDetail.serviceFee&&<><dt>{t("服务费（0.5%）")}</dt><dd>{formatAmount(BigInt(historyDetail.serviceFee))} QTC</dd><dt>{t("服务费收款地址")}</dt><dd>{historyDetail.serviceFeeAddress}</dd></>}
 {historyDetail.estimatedNetworkFee&&<><dt>{t("提交时预计网络费（实际以链上为准）")}</dt><dd>{formatAmount(BigInt(historyDetail.estimatedNetworkFee))} QTC</dd></>}
 {historyDetail.includedHeight!==undefined&&<><dt>{t("入块高度")}</dt><dd>{historyDetail.includedHeight.toLocaleString()}</dd></>}{historyDetail.finalizedHeight!==undefined&&<><dt>{t("上次查询的最终确认高度")}</dt><dd>{historyDetail.finalizedHeight.toLocaleString()}</dd></>}</dl>
 {historyError&&<p className="notice error" role="alert">{t(historyError)}</p>}
 <Button className="secondary full" disabled={!!historyTracking||record?.hash===historyDetail.hash&&tracking} onClick={()=>void queryHistory(historyDetail)}><RefreshCw size={16} className={historyTracking?'spin':''}/>{record?.hash===historyDetail.hash&&tracking?t("当前交易正在自动查询"):historyTracking?t("正在查询，关闭详情可停止"):t("重新查询链上结果")}</Button>
 <a className="receipt-link" href={explorer(historyDetail.from)} target="_blank" rel="noreferrer">{t("在区块浏览器查看账户")}<ArrowUpRight size={15}/></a></>:<p className="subtle">{t("这条本地记录已不可用，请通过区块浏览器查询。")}</p>}
 ...
 <Dialog open={!!prepared} onOpenChange={open=>{if(!open&&!busy){setPrepared(null);setAck(false);}}}><DialogContent showCloseButton={!busy} className="review-dialog" onEscapeKeyDown={e=>{if(busy)e.preventDefault();}} onPointerDownOutside={e=>{if(busy)e.preventDefault();}}><DialogHeader><DialogTitle>{t("请核对这笔转账")}</DialogTitle><DialogDescription>{t("这是普通转账，成功入块后不能通过本工具撤销。")}</DialogDescription></DialogHeader>{prepared&&<><div className="receipt-amount">{formatAmount(prepared.amount)} <span>QTC</span></div><dl className="review-details"><dt>{t("收款地址")}</dt><dd>{prepared.to}</dd><dt>{t("付款地址")}</dt><dd>{prepared.from}</dd><dt>{t("服务费（0.5%，额外收取）")}</dt><dd>{formatAmount(prepared.serviceFee)} QTC</dd><dt>{t("服务费收款地址")}</dt><dd>{prepared.serviceFeeAddress}</dd><dt>{t("预计网络手续费")}</dt><dd>{formatAmount(prepared.fee)} QTC</dd><dt>{t("预留手续费（估算 + 10%）")}</dt><dd>{formatAmount(prepared.maxFee)} QTC</dd><dt>{t("预计总支出（到账金额 + 服务费 + 网络费）")}</dt><dd>{formatAmount(transferDebit(prepared.amount)+prepared.fee)} QTC</dd><dt>{t("余额至少需预留（含网络费缓冲）")}</dt><dd>{formatAmount(transferDebit(prepared.amount)+prepared.maxFee)} {t("QTC，另保留账户最低余额")}</dd><dt>{t("网络")}</dt><dd>Quantus Mainnet</dd></dl><p className="micro">{t("服务费按到账金额的 0.5% 计算，向上取整至最小单位。实际网络费由入块时状态计算。转账或收费失败时两者均回滚，网络费可能仍扣除。预览 2 分钟后失效。")}</p><label className="confirmation"><Checkbox checked={ack} onCheckedChange={v=>setAck(v===true)} disabled={!!busy}/><span>{t("我已核对收款地址、到账金额，并同意支付上述 0.5% 服务费及网络手续费。")}</span></label><Button className="primary full" disabled={!ack||!!busy} onClick={send}>{busy?<><LoaderCircle size={18} className="spin"/>{t("正在核对、签名并提交")}</>:<>{t("确认签名并发送")}<ArrowUpRight size={18}/></>}</Button></>}</DialogContent></Dialog>
 </div>;
}
