...
export default function Home(){
 const [view,setView]=useState<'transfer'|'mining'>(()=>typeof window!=='undefined'&&window.location.hash==='#mining'?'mining':'transfer');
 useEffect(()=>{const changed=()=>setView(window.location.hash==='#mining'?'mining':'transfer');window.addEventListener('hashchange',changed);return()=>window.removeEventListener('hashchange',changed);},[]);
 ...
 <main className="workspace"><nav className="desk-navigation" aria-label="工具导航"><a href="#transfer" aria-current={view==='transfer'?'page':undefined}>QTC 转账{record&&!complete(record)?' · 进行中':''}</a><a href="#mining" aria-current={view==='mining'?'page':undefined}>挖矿成本计算器</a></nav><ReferralBanner/><div className="page-heading"><div><span className="eyebrow">{view==='mining'?'QUANTUS / MINING':'QUANTUS / TRANSFER'}</span><h1>{view==='mining'?'算清每一枚 QTC 的成本':'QTC 主网转账'}</h1>{view==='mining'&&<p className="mining-subtitle">配置你的设备，看看当前算力下的产量、成本与盈亏。</p>}</div><div className="heading-links"><a className="author-link" href="https://x.com/kkmoat" target="_blank" rel="noopener noreferrer" aria-label="作者 X：@kkmoat，在新标签页打开">作者 X：<strong>@kkmoat</strong><ArrowUpRight size={17} aria-hidden="true"/></a><span className="session-label"><LockKeyhole size={16}/> {view==='mining'?'本地计算 · 无需钱包':'本地签名'}</span></div></div>
