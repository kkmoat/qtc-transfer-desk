...
export default function Home(){
 useLanguage();
 const [view,setView]=useState<'transfer'|'mining'|'encrypted'>(()=>typeof window!=='undefined'?(window.location.hash==='#mining'?'mining':window.location.hash==='#encrypted'?'encrypted':'transfer'):'transfer');
 useEffect(()=>{const changed=()=>setView(window.location.hash==='#mining'?'mining':window.location.hash==='#encrypted'?'encrypted':'transfer');window.addEventListener('hashchange',changed);return()=>window.removeEventListener('hashchange',changed);},[]);
 ...
 <main className="workspace"><nav className="desk-navigation" aria-label={t("工具导航")}><a href="#transfer" aria-current={view==='transfer'?'page':undefined}>{t("QTC 转账")}{record&&!complete(record)?t(" · 进行中"):''}</a><a href="#encrypted" aria-current={view==='encrypted'?'page':undefined}>{t('加密账户')}</a><a href="#mining" aria-current={view==='mining'?'page':undefined}>{t("挖矿成本计算器")}</a></nav><ReferralBanner/><div className="page-heading"><div><span className="eyebrow">{view==='mining'?'QUANTUS / MINING':view==='encrypted'?'QUANTUS / WORMHOLE':'QUANTUS / TRANSFER'}</span><h1>{view==='mining'?t("算清每一枚 QTC 的成本"):view==='encrypted'?t("加密账户"):t("QTC 主网转账")}</h1>{view==='mining'&&<p className="mining-subtitle">{t("配置你的设备，看看当前算力下的产量、成本与盈亏。")}</p>}</div><div className="heading-links"><a className="author-link" href="https://x.com/kkmoat" target="_blank" rel="noopener noreferrer" aria-label={t("作者 X：@kkmoat，在新标签页打开")}>{t("作者 X：")}<strong>@kkmoat</strong><ArrowUpRight size={17} aria-hidden="true"/></a><span className="session-label"><LockKeyhole size={16}/> {view==='mining'?t("本地计算 · 无需钱包"):t("本地签名")}</span></div></div>
