...
- 加密账户恢复在独立 Worker 中持有官方派生的自擦除 seed，secret/first hash 不返回主线程。页面只获取派生地址及用于公开状态查询的 nullifier。加密账户离开对应工具页面时也锁定。当前浏览器不提供 Wormhole 转出，Worker 明确拒绝证明请求。
...
语言选择另以 `qtc-language-v1` 保存 `zh` 或 `en`，本地静态翻译不调用外部服务。切换语言不重新派生钱包、不改付款金额或费用、不提交交易。加密账户的 seed、地址列表及扫描余额不持久保存。
