...
import type { OpenWallet } from '../quantus/signer.ts';
export interface EncryptedSession { address: string; accountId: number[]; path: string }
export interface ProofProgress { stage:string;completed?:number;total?:number }
import type { WormholeProofRequest, WormholeCheckedSummary, WormholeVerifiedProof } from './withdraw.ts';
...
export class EncryptedWallet {
  ...
  private id = 0;
  private pending = new Map<number, {resolve:(v:unknown)=>void;reject:(e:Error)=>void;timer:ReturnType<typeof setTimeout>;onProgress?:(progress:ProofProgress)=>void}>();
  ...
  constructor(onClosed?: (reason:string)=>void) {
    ...
    this.worker.onmessage = event => {
      ...
      if (!entry) return;
      if(event.data.progress){entry.onProgress?.(event.data.progress as ProofProgress);return;}
  ...
  private request<T>(data:Record<string,unknown>,onProgress?:(progress:ProofProgress)=>void):Promise<T> {
    return new Promise((resolve,reject) => {
      if (this.closed) { reject(new Error('钱包已锁定，请重新打开。')); return; }
      const id = ++this.id;
      const timer = setTimeout(() => this.close('本地签名操作超时，钱包已自动锁定。',true),data.type==='wormhole-prove'?300_000:90_000);
      this.pending.set(id,{resolve:v=>resolve(v as T),reject,timer,onProgress});
  ...
  normal(accountIndex:number) { return this.request<OpenWallet>({type:'wormhole-normal',accountIndex}); }
  check(request:WormholeProofRequest) { return this.request<WormholeCheckedSummary>({type:'wormhole-check',request}); }
  prove(request:WormholeProofRequest,onProgress?:(progress:ProofProgress)=>void) { return this.request<WormholeVerifiedProof>({type:'wormhole-prove',request},onProgress); }
