...
- qtc123 的所有页面在前台可见时读取同源 `/api/lucky-bag/enter`。后台开启活动后，每个正在访问的可见页面都会看到福袋邀请；查看邀请不会占用名额，只有点击“打开福袋”并由 `/api/lucky-bag/reserve` 成功响应后才取得预留。随后扫码添加 kk 微信、填写完整 Quantus 收款地址和微信号，勾选资料用途说明并提交登记。微信二维码和官方钱包来自现有导航资源。
- 数量与总预算分别设置，例如 **10 袋合计 0.1000 QTC**；奖额随机、每份至少 0.0001 QTC、保留四位小数，合计严格等于预算。预算是活动配置，不会自动从钱包扣款。
- 点击打开后按服务器返回的固定到期时间预留 3 分钟。关闭填写窗口只会将入口收进页面右下角，倒计时继续且可再次打开；刷新页面会恢复同一预留，隐藏标签页或离开页面也不会释放。3 分钟内未提交即永久作废，该份金额不会重新分配给其他访客。邀请页在尚未预留时关闭后，本页面内不再重复弹出；新页面仍可重新读取活动状态。登记成功才计入已领取，关闭活动立即停止新预留。
- 后台 `https://admin.qtc123.com` 与主站分开托管、需要登录。可创建、开关或删除活动，也可查询和删除领取记录；删除会清除领取资料，但不会把已占用的名额和金额重新发放。活动默认关闭。
