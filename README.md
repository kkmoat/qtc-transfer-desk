...
- qtc123 的所有页面在前台可见时读取同源 `/api/lucky-bag/enter`。只有后台已开启且当前请求成功取得预留名额的访客才看到福袋。打开后扫码添加 kk 微信、填写完整 Quantus 收款地址和微信号，勾选资料用途说明并提交登记。微信二维码和官方钱包来自现有导航资源。
- 数量与总预算分别设置，例如 **10 袋合计 0.1000 QTC**；奖额随机、每份至少 0.0001 QTC、保留四位小数，合计严格等于预算。预算是活动配置，不会自动从钱包扣款。
- 首批名额先到先得，预留 5 分钟。主动关闭会放弃本次开启中的资格；刷新或离页只释放预留，之后仍可重新排队。等待页每 15 秒更新，只有访客自己的可见页面请求才会取得预留。登记成功才计入已领取。关闭活动立即停止新登记；再次开启时，未领取访客进入新一轮队列。
