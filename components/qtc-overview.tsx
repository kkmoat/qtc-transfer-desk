...
import {fetchPriceSnapshot,SAFETRADE_LINK,type PriceSnapshot} from '@/lib/overview/price';
...
export function QtcOverview({active}:{active:boolean}){
 ...
 async function refresh(){
  ...
  await Promise.all([update(fetchSupplySnapshot,setSupply,setSupplyError),update(fetchPriceSnapshot,setPrice,setPriceError)]);
 ...
 return <section className="overview" aria-label={t('QTC 总览')}>
  ...
  <article className="overview-market-card">
   ...
   <div className="overview-market-footer"><div aria-live="polite">{priceError&&<p className="overview-feed-error">{t(priceError)}</p>}{price?<p>{t('报价获取时间：{0}；不是最近成交发生时间。',when(price.fetchedAt))}</p>:<p>{loading&&!priceError?t('正在连接 SafeTrade 公开行情…'):t('暂未取得可验证报价。')}</p>}</div><a href={SAFETRADE_LINK} target="_blank" rel="noopener noreferrer">{t('打开 SafeTrade 行情')}<ArrowUpRight size={16}/></a></div>
  </article>
